...
| [obsidian：笔记同步](../panghu_chat/obsidian/README.md) | **方案已换（2026-09-24）**：放弃 Selkies 浏览器工作台，改为 CouchDB + Self-hosted LiveSync，服务端用 `livesync-cli mirror` 物化出纯 Markdown 供下游只读挂载；认证用 JWT。目录内的 [runtime-state.md](../panghu_chat/obsidian/runtime-state.md) 记录了促成这次改动的实测证据（浏览器里是 Selkies 串流桌面、vault 落错 PVC）。⚠️ 本目录下另有 [obsidian-deployment-research.md](obsidian-deployment-research.md)（2026-09-21，从未被本索引收录），它当时推荐的 CouchDB + LiveSync 方向**现在被采纳**，但它把浏览器工作台列为可选补充的判断已在 09-23 被推翻过一轮 |
