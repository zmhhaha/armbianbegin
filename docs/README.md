...
| 文档 | 主题 |
|---|---|
| [service-git-access.md](service-git-access.md) | **让 DSH 与 Hermes 能改代码并推送的受控 Git 访问调研（2026-09-25）**。核心结论：**凭据在服务里藏不住**（agent 与 git 是同一个 uid 10000 进程），能做的是压缩半径——细粒度 PAT 只授 `Contents: Read and write`、只限必需仓库、设到期日；**不要给 `Workflows`**（能推 workflow 文件 = 往那个仓库的 CI 注入可执行代码）。另有两处纠偏：GitHub 的 **SSH 对公开仓库也要认证**（不存在匿名 SSH clone，所以那次 `Permission denied (publickey)` 是唯一可能的结果），而 `user.name`/`user.email` 与 GPG **都不是凭据**。§六 是动手前确认清单 |
...
| [obsidian：笔记同步](../panghu_chat/obsidian/README.md) | **已部署（2026-09-25）**：CouchDB + Self-hosted LiveSync，设备侧原生 Obsidian（JWT 认证），服务端用 `livesync-cli` 的 daemon 模式物化出纯 Markdown 到 CephFS RWX 卷，供下游只读消费。云端密文、端到端加密。目录内的 [deployment-notes.md](../panghu_chat/obsidian/deployment-notes.md) 是**踩坑与排查手册**（19 条实测坑，出问题先看它）；[downstream-distribution.md](../panghu_chat/obsidian/downstream-distribution.md) 设计笔记如何按 corpus 分给不同消费方（各服务独立 collection、Hermes/DSH 的共享记忆）。⚠️ 本目录下另有 [obsidian-deployment-research.md](obsidian-deployment-research.md)（2026-09-21，从未被本索引收录），它当时推荐的 CouchDB + LiveSync 方向**最终被采纳**，但域名与 TunnelRoute 效力两点已就地更正 |
