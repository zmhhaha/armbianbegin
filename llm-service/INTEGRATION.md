...
**调用方的 HTTP 超时也要跟着放，否则会在服务端还在生成时就 abort。**

这是上面那个问题在客户端的同一面。推理模型的耗时方差比直连大得多，原来按「快问快答」估的超时
（8-20 秒）在长文 / 结构化输出上会直接穿掉。而且**表现具有欺骗性**：

```
调用方日志   →  AbortError / 超时（看起来像服务端挂了）
llm-service  →  200 OK，latency 35s（它不知道你已经走了，还在继续生成）
```

两边都会留下记录，但结论完全相反 —— 只看服务端会以为一切正常，只看调用方会以为服务不可用。
**判断方法：拿 llm-service 记的 `latency_ms` 和你的超时值比。** 观测 35s 而超时 20s，就是你误杀了一次成功的调用。

参考取值（2026-09 实测）：

| 场景 | 实测耗时 | 建议超时 |
|---|---|---|
| 单轮对话（盘问、发言、审查） | 3–10s | 60s |
| 结构化输出 / 一次多个对象（GuanLiao 整日批量） | 35s+ | 120s |

放大的代价只是「真挂住时多等一会儿」，而放小的代价是**已经付费、已经生成完的结果被自己丢掉**。
