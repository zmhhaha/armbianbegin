...
# The generic UI image is deliberately reused as :latest. Force replacement
# after a rebuild because apply alone leaves an unchanged Pod template intact.
kubectl rollout restart deployment/ui -n "${AGENT_NAME}" ${K}
kubectl rollout status deployment/ui -n "${AGENT_NAME}" ${K} --timeout=180s
