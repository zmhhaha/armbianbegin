...
deploy_proxy() {
    echo ""
    echo "=== 部署 oauth2-proxy 实例（research-agent + scientific-agent）==="
    for target in research-agent scientific-agent; do
        echo "  ── 部署 ${target} ──"
...
case "${1:-}" in
    ...
    --help)
        ...
        echo "  --deploy        拉取镜像 + 部署 Casdoor + 部署 oauth2-proxy 两个实例"
        echo "  --deploy-proxy  仅部署/更新 oauth2-proxy 实例（research-agent + scientific-agent）"
        echo ""
        echo "  oauth2-proxy 实例:"
