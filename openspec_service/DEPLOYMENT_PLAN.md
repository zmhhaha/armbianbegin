...
├── k8s/                       # namespace、Deployment、Service、PVC、ExternalSecret、TunnelRoute
└── scripts/bootstrap-gitea-project.sh
```

## 7. 部署步骤

1. 在 Casdoor 创建 `openspec-api` 应用，确认 issuer、audience 和 JWKS；在 Gitea 创建 `openspec` 组织并准备受限服务账号。
2. 创建 PostgreSQL 数据库/用户，在 Vault 写入 `secret/openspec/service`，确认 ExternalSecret 能同步 `openspec-service-secrets`。
3. 构建 ARM64 镜像并推送私有 Registry。
4. 应用 `k8s/`，创建 namespace、PVC、ExternalSecret、Deployment、ClusterIP Service 和 Cloudflare TunnelRoute。
